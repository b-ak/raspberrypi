...
int pll_frequency_setup(unsigned int frequency)
{
   ...
   if(CSL_SOK != status)
   {
    ...
    if(status != CSL_SOK)
	{
	    debug(("TEST FAILED: PLL get config... Failed.\n"));
		debug(("Reason: PLL_getConfig failed. [status = 0x%x].\n", status));
		return(status);
	}

    debug(("REGISTER --- CONFIG VALUES\n"));

    debug(("PLL_CNTRL1   %04x --- %04x\n",pllCfg1.PLLCNTL1,hPll->pllConfig->PLLCNTL1));
    debug(("PLL_CNTRL2   %04x --- %04x Test Lock Mon will get set after PLL is up\n",
                                pllCfg1.PLLCNTL2,hPll->pllConfig->PLLCNTL2));
    debug(("PLL_CNTRL3   %04x --- %04x\n",pllCfg1.PLLINCNTL,hPll->pllConfig->PLLINCNTL));
    debug(("PLL_CNTRL4   %04x --- %04x\n",pllCfg1.PLLOUTCNTL,hPll->pllConfig->PLLOUTCNTL));
